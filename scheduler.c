...
Process* getNextProcess() 
{
    for(int i = 0; i < numPriorities; ++i) 
    {
        if( !isEmpty(priorityProc[i]) ) return getQueueHead(priorityProc[i]);
    }
...
void scheduler() 
{
    ...
    *scheduler_pid = getpid();
    ...
    int currentSecond = 0, prevSecond = 0;
    ...
    while(totalProcesses == 0 || totalProcesses != finishedProcesses) 
    {
        ...
        if (executingRealTimeProcess) 
        {
            ...
            else 
            {
                ...
                if (curProcess->type == 3 && curProcess->duration + curProcess->start == currentSecond) 
                {
        ...
        if (!executingRealTimeProcess) 
        {
            ...
            if (nProcess != NULL) 
            { // there is an enqueued process to be executed
                if(curProcess != NULL) 
                {
                ...
                else 
                {
                    ...
                    changedProcess = 1;
                    if (curProcess->priority == -1) 
                    { // save start time for round robin processes
